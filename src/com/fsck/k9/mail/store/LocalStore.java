...
public class LocalStore extends Store implements Serializable {
    ...
    public Message[] searchForMessages(MessageRetrievalListener listener, String[] queryFields, String queryString,
        ...
        return getMessages(
                   ...
                   "SELECT "
                   + GET_MESSAGES_COLS
                   + "FROM messages WHERE (empty IS NULL OR empty = 0) AND deleted = 0 " + whereClause.toString() + " ORDER BY date DESC"
    ...
    public class LocalFolder extends Folder implements Serializable {
            ...
            try {
                return database.execute(false, new DbCallback<Integer>() {
                    ...
                    public Integer doDbWork(final SQLiteDatabase db) throws WrappedException {
                        ...
                        Cursor cursor = null;
                        try {
                            cursor = db.rawQuery("SELECT COUNT(*) FROM messages WHERE (empty IS NULL OR empty = 0) AND deleted = 0 and folder_id = ?",
                                                 new String[] {
                    ...
                    public Message[] doDbWork(final SQLiteDatabase db) throws WrappedException, UnavailableStorageException {
                        try {
                            ...
                            return LocalStore.this.getMessages(
                                       ...
                                       + "FROM messages WHERE (empty IS NULL OR empty = 0) AND "
        ...
        private void clearMessagesWhere(final String whereClause, final String[] params)  throws MessagingException {
            ...
            Message[] messages  = LocalStore.this.getMessages(
                                      null,
                                      this,
                                      "SELECT " + GET_MESSAGES_COLS + "FROM messages WHERE (empty IS NULL OR empty = 0) AND (" + whereClause + ")",
                                      params);
